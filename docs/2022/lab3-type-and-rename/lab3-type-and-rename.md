...
- 对于不存在语义错误的输入文件，在本部分不输出任何内容
    - 请注意，请输出最本质的错误（最本质的错误是指由于当前错误而可能引发的其他错误），对其进行合理的恢复后继续分析后续错误
      ...
      - 我这里举几个例子来说明什么是本质的错误：
        ...
        + 例1:
          ...
          ```
          Error type 2 at Line 2: ……
          ```
    
          解释：函数`b`未定义，所以这里就不需要再抛出返回值异常。
        ...
        + 例2：
          ...
          ```
          Error type 3 at Line 1: ……
          ```
    
          解释：变量重复定义，所以报`Error type 3`，此时你要抛弃后一个`int i`，认为定义了一个`int test(int)`类型的函数，所以第二行函数调用没有错误。
        ...
        + 例3：
          ...
          ```
          Error type 4 at Line 5: ……
          ```
    
          解释：扫描过第一个函数，符号表记录了`int test(int)`，当扫描第二个函数时，会发现名为`test`的函数已经在符号表里定义过，报`Error type 4`，同时抛弃掉第二个`test`函数，不再继续扫描此函数。
...
- 本部分你需要对指定的字符重命名，要求如下
    ...
    - 重命名与`IDEA`选中某个变量然后重命名的效果一致，需要为所有与选中变量生命周期一致的同名变量重命名。选中的变量可能包括如下情况：
...
## 实验帮助
点击[这里](2022/lab3-type-and-rename/help.md)查看实验三指导文档
